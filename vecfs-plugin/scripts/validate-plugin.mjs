...
/**
 * Validates the VecFS Cursor plugin structure.
 * Run from vecfs-plugin/ directory: node scripts/validate-plugin.mjs
 */
...
async function main() {
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    const sourcePath = pluginRoot ? `${pluginRoot}/${entry.source}`.replace(/\/+/g, "/") : entry.source;
    if (!sourcePath || typeof entry.source !== "string") {
